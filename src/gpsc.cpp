...
std::vector<VDPGPSClient::distance_t>
VDPGPSClient::get_min_distance(ldmmap::LDMMap* LDM) {
    ...
    VRU_position_latlon_t ped_pos = getPedPosition ();
    ...
    VRU_position_XYZ_t pos_ped = convertLatLontoXYZ_TM(ped_pos,lon0);
    ...
    for(std::vector<ldmmap::LDMMap::returnedVehicleData_t>::iterator it = selectedStations.begin (); it!=selectedStations.end (); ++it){
        ...
        VRU_position_latlon_t pos_node;
        VRU_position_XYZ_t pos_node_xyz;
