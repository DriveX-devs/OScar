...
bool ECResponse::signatureVerification(const std::string &tbsData_hex, GNecdsaNistP256 &rValue, const std::string &sValue, GNecdsaNistP256 verifyKeyIndicator)
{
  ...
  // Usa il certificato EA letto da PKI_info.ini (eaCert1..3 concatenati)
  if (eaCert1.empty() || eaCert2.empty() || eaCert3.empty()) {
    // fallback: prova a leggere dal file ini se non già caricato
    readIniFile();
  }
  std::string signIdentifierSelf = eaCert1 + eaCert2 + eaCert3;
...
void ECResponse::readIniFile() {
    ...
    eaCert1 = reader.Get("ECinfo", "eaCert1", "");
    eaCert2 = reader.Get("ECinfo", "eaCert2", "");
    eaCert3 = reader.Get("ECinfo", "eaCert3", "");

    // Load RFC5480 keys from INI (private split in two parts)
